...
impl Mention {
    fn new(
        ...
        match (signature, alt_text) {
            (SignifiedButNotProvided, AltText::SignifiedAndProvided(fragment)) => {
                Some(Placeholder { fragment })
            }
            (
                SignifiedAndProvided(signature),
                AltText::NotSignified | AltText::SignifiedButNotProvided,
            ) => {
    ...
    fn new(state: &mut InlineState, mention_signature: String) -> (Mentionable, u32) {
        let mentionable = {
            if let Some(doi) = mention_signature.strip_prefix("doi:") {
                Mentionable::Paper(Paper {
                    doi: doi.to_string(),
                })
            } else if let Ok(_url) = url::Url::parse(mention_signature.as_str()) {
                Mentionable::Link(Link {
                    url: mention_signature,
                })
            } else if mention_signature.contains("-by-") {
                let mut segments: Vec<&str> =
                    mention_signature.split("-by-").map(|x| x.trim()).collect();
                Mentionable::Book(Book {
                    primary_author: segments.pop().unwrap().to_string(),
                    title: segments.join(" "),
                })
            } else {
                let mut names = mention_signature
                    .split(' ')
                    .map(|x| x.trim())
                    .collect::<Vec<&str>>();
        ...
        let occurance = *state
            .root_ext
            .get_or_insert_default::<MentionableOccurances>()
            .0
    ...
    fn new(state: &mut InlineState) -> Option<MentionSignature> {
        ...
        while state.pos < state.pos_max {
            if state.src.get(state.pos..state.pos + 1)? == "|"
                || state.src.get(state.pos..state.pos + 2)? == "]]"
            {
                if state.pos <= start {
                    return Some(MentionSignature::SignifiedButNotProvided);
                } else {
                    return Some(MentionSignature::SignifiedAndProvided(
                        state.src.get(start..state.pos)?.to_string(),
                    ));
                }
            }
        ...
        while state.pos < state.pos_max {
            if state.src.get(state.pos - 1..state.pos + 1)? == "]]" {
                ...
                if final_pos <= start + 2 {
                    ...
                    return Some(AltText::SignifiedAndProvided(Fragment {
                        start,
                        end: state.pos - 2,
                    }));
                }
